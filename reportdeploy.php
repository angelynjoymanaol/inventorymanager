...
<a style="color:green;" href="viewdeployequipment.php"><p style="font-family:Agency FB;color:black;font-size:50px;line-height:120%;text-align:left;margin-left:250px;margin-top:-30px"> << </a>
V I E W &emsp; D E P L O Y E D&emsp; E Q U I P M E N T


<?php
echo "<a href=\"reportdeploy2.php?begin={$_POST['begin']}&&last={$_POST['last']}\"><img src=\"pdf.png\"  /></a>";
...
$TotalRec = mysql_result(mysql_query("SELECT COUNT(*) FROM deploy WHERE date >= '$_POST[begin]'  AND date <= '$_POST[last]'"), 0);

$select = "SELECT * FROM deploy WHERE date >= '$_POST[begin]'  AND date <= '$_POST[last]' LIMIT $start, $perpage";
...
{
  echo '<a href='."./reportdeploy.php?id=" . ($start - $perpage) . '>'."Previous".'</a>';
}?>
...
{
  echo '<a href='."./reportdeploy.php?id=" . ($start + $perpage) . '>'."Next".'</a>';
}
