...
$TotalRec = mysql_result(mysql_query("SELECT COUNT(*) FROM retire WHERE date >= '$_POST[begin]'  AND date <= '$_POST[last]'"), 0);
...
{
  echo '<a href='."./reportretire.php?id=" . ($start - $perpage) . '>'."Previous".'</a>';
}?>
...
{
  echo '<a href='."./reportretire.php?id=" . ($start + $perpage) . '>'."Next".'</a>';
}
