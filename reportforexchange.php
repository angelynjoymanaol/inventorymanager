...
<a style="color:green;" href="viewforexchange.php"><p style="font-family:Agency FB;color:black;font-size:50px;line-height:120%;text-align:left;margin-left:250px;margin-top:-30px"> << </a>
...
$TotalRec = mysql_result(mysql_query("SELECT COUNT(*) FROM forexchange WHERE date >= '$_POST[begin]'  AND date <= '$_POST[last]'"), 0);

$select = "SELECT * FROM forexchange WHERE date >= '$_POST[begin]'  AND date <= '$_POST[last]' LIMIT $start, $perpage";
...
{
  echo '<a href='."./reportforexchange.php?id=" . ($start - $perpage) . '>'."Previous".'</a>';
}?>
...
{
  echo '<a href='."./reportforexchange.php?id=" . ($start + $perpage) . '>'."Next".'</a>';
}
